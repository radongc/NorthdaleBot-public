...
using GreyMagic;
using NorthdaleBotWpf.Hook.DirectX;
using NorthdaleBotWpf.Memory;
...
namespace NorthdaleBotWpf.Hook
{
    ...
    public class AsmHookTemp
    {
        ...
        public AsmHookTemp(Process wowProc)
        {
            Process = wowProc;
        ...
        public void InstallHook()
        {
            try
            {
                ...
                if (Memory.IsProcessOpen)
                {
                    ...
                    PlayerPtr = Memory.AllocateMemory(0x4);

                    //_extReader.Write(_injectionAddress, 0);
                    Memory.Write<ulong>(PlayerPtr, ObjectManager.PlayerObject.BaseAddress);
                    ...
                    if (Memory.Read<byte>(_dx3D.HookPtr) == 0xEB && (_injectedCode == IntPtr.Zero || _addresseInjection == IntPtr.Zero))
                    {
                        ...
                        Console.WriteLine("Hook already installed. Disposing...");
                    }
                    ...
                    //Console.WriteLine("Original EndSceneBytes = ({0})", sBytes);
            ...
            lock (_executeLockObject)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            //Console.WriteLine("dwAddress {0}", dwAddress);
